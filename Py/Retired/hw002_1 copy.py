...
def FindBestSplit(D,C, Xi):
    ...
    for feature in Xi:
        ...
        for x in range(len(dict_of_gains[feature])):
            ...
            if GainRatio(list(dict_of_gains[feature].items())[x][1]) > comp_max :
                #print('^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^')
                #print(GainRatio(list(dict_of_gains.items())[0][1]))
                best_feature = feature
                comp_max = GainRatio(list(dict_of_gains[feature].items())[x][1])
                entropy_max = entropy(list(dict_of_gains[feature].items())[x][1])
                info_gain = InfoGain(list(dict_of_gains[feature].items())[x][1])
                max = list(dict_of_gains[feature].items())[x][0]
                c_num=x
...
class Tree():
    ...
    def MakeSubtree(self, D, Xi):
        ...
        #     make internal node N
            ...
            #print(sortData(D,best_feature))
            for sub in listSplit(sortData(D,best_feature),S[0]):
                #print(sub.to_string())
                #print(sub)
                Dk = sub
